...
const Post = ({ post }) => {
	...
	const {mutate: deletePost, isPending: isDeleting } = useMutation({
		...
		onSuccess: () => {
			...
			//invalidate query to refatch the data
	...
	const { mutate: likePost, isPending: isLiking } = useMutation({
		...
		onSuccess: (updatedLikes) => {
			...
			//this is not good ux for liking bc it will refetch the whole app
			// queryClient.invalidateQueries({queryKey: ["posts"]})
			//instead will update the post cache 
			queryClient.setQueriesData(["posts"], (oldData) => {
				...
				// Case 2: oldData is object with posts array (e.g. { posts: [...] })
				...
				// Return unchanged if unexpected shape
				return oldData;
			})
		},
		onError: (error) => {
